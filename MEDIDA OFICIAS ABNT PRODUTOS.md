Implementação Técnica para o Cursor
Para que o sistema busque a referência correta e não dê erro, vamos criar um "Dicionário Inteligente". Copie o código abaixo e peça para o Cursor criar o arquivo src/lib/standards/abnt-data.ts.

Isso resolve o problema de "adivinhação". Se a IA identificar "Camiseta Infantil 6", ela vai ler bust: 61 diretamente deste arquivo.
...
export function getStandardMeasurements(audience: string, size: string) {
  const table = ABNT_STANDARDS[audience];
  ...
  const converter = SIZE_CONVERTER[audience as keyof typeof SIZE_CONVERTER];
  if (converter && converter[size as keyof typeof converter]) {
    const numericSize = converter[size as keyof typeof converter];
    return table[numericSize];
  }

  return null;
}
3. Melhor forma de fazer isso para não dar erro
A "estratégia à prova de falhas" para o seu desenvolvedor (Cursor) é:

Copie o código acima e crie o arquivo abnt-data.ts.

No Editor de Medidas (SmartMeasurementEditor.tsx), adicione esta lógica:

Quando o usuário seleciona um tamanho (ex: "6" Infantil):

O sistema chama getStandardMeasurements('KIDS', '6').

O sistema recebe { bust: 61, waist: 58, ... }.

Ação: O sistema preenche os inputs com esses valores automaticamente e exibe um selo "Padrão ABNT".

Visual: As setas de medida (SVG) se ajustam na imagem apenas para mostrar onde é o busto, mas o valor numérico vem desta tabela, garantindo que nunca haverá "alucinação" de valores absurdos.