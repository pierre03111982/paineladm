/**
 * Serviço de Prompt Mestre VTO (Virtual Try-On)
 * 
 * Fornece o prompt mestre otimizado para Gemini 2.5 Flash Image
 * baseado no documento PROMPT_LOOK_CRIATIVO.md
 * 
 * O prompt é usado diretamente no Gemini Flash Image para gerar
 * imagens de Virtual Try-On com máxima fidelidade da pessoa e produtos.
 */

/**
 * Prompt Mestre Definitivo (VTO) - Gemini 2.5 Flash Image
 * Versão: 2.0 (Final VTO Modular)
 * Baseado em: E:\projetos\paineladm\docs\PROMPT_LOOK_CRIATIVO.md
 * 
 * Este prompt é otimizado para a API Gemini 2.5 Flash Image, com foco em:
 * - Fidelidade inalterável da pessoa (PRIORIDADE 1)
 ...
 * 
 * Este prompt é usado diretamente no Gemini Flash Image, sem necessidade
 * de passar por outro modelo Gemini para otimização.
 * 
 * O prompt mestre está baseado em: E:\projetos\paineladm\docs\PROMPT_LOOK_CRIATIVO.md
 * Versão: 2.0 (Final VTO Modular)
 * 
 * @param userImageUrl URL da imagem da pessoa (IMAGEM_PESSOA)
 * @param productImageUrls Array de URLs dos produtos (máximo 3)
 * @returns O prompt mestre completo para Virtual Try-On
 */
export async function generateImagenPrompt(
  userImageUrl: string,
  productImageUrls: string[]
): Promise<string> {
  console.log("[GeminiPrompt] Usando prompt mestre VTO diretamente", {
    userImageUrl: userImageUrl.substring(0, 100) + "...",
    productCount: productImageUrls.length,
    maxProducts: Math.min(productImageUrls.length, 3),
  });

  // Retornar o prompt mestre diretamente
  // O Gemini Flash Image receberá este prompt junto com as imagens
  // na ordem: IMAGEM_PESSOA (primeira), depois produtos (máximo 3)
  return MASTER_PROMPT_TEMPLATE;
}

/**
 * Prompt padrão de fallback (caso necessário no futuro)
 * 
 * NOTA: Atualmente não é usado, pois sempre retornamos o MASTER_PROMPT_TEMPLATE
 * diretamente. Mantido para compatibilidade futura.
 */
function getDefaultPrompt(): string {
  // Retornar o prompt mestre mesmo como fallback
  return MASTER_PROMPT_TEMPLATE;
}
