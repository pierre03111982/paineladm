      ...
      <section className="grid gap-6 lg:grid-cols-4">
        <div className="rounded-2xl border border-indigo-400/40 bg-indigo-500/10 p-6">
          ...
          <div className="mt-4 space-y-2">
            <div>
              ...
              <p className="text-xs text-indigo-200/80">Hoje</p>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <p className="text-lg font-semibold text-white">{data.metrics.experimentWeek}</p>
                <p className="text-indigo-200/80">Semana</p>
              </div>
              <div>
                <p className="text-lg font-semibold text-white">{data.metrics.experimentMonth}</p>
                <p className="text-indigo-200/80">Mês</p>
              </div>
            </div>
            {data.metrics.planoLimite !== null && data.metrics.planoLimite !== undefined ? (
              <p className="text-xs text-indigo-200/60">Plano: {data.metrics.experimentMonth} / {data.metrics.planoLimite}</p>
            ) : (
              <p className="text-xs text-indigo-200/60">Plano: Ilimitado</p>
            )}
          </div>
          <div className="mt-6">
        ...
        <div className="rounded-2xl border border-zinc-800/70 bg-zinc-900/70 p-6">
          <div className="flex items-center gap-2 text-xs text-zinc-400">
            ...
            <span>Novos Clientes (Dia)</span>
          </div>
          <p className="mt-3 text-2xl font-semibold text-white">
            {data.metrics.novosClientesDia} / {data.metrics.totalClientes}
          </p>
          <p className="mt-4 text-xs text-zinc-500">
            Total de clientes cadastrados: {data.metrics.totalClientes}
          </p>
        ...
        <div className="rounded-2xl border border-emerald-400/40 bg-emerald-500/10 p-6">
          <div className="flex items-center gap-2 text-xs text-emerald-200">
            ...
            <span>Likes Acumulados (Total Geral)</span>
          </div>
          <p className="mt-3 text-2xl font-semibold text-white">
          ...
          <div className="mt-4 space-y-2">
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="flex items-center gap-1 text-emerald-100/80">
                <ThumbsUp className="h-3 w-3" />
                <span>{data.metrics.likesDia}</span>
              </div>
              <div className="flex items-center gap-1 text-emerald-100/80">
                <ThumbsUp className="h-3 w-3" />
                <span>{data.metrics.likesSemana}</span>
              </div>
              <div className="flex items-center gap-1 text-emerald-100/80">
                <ThumbsUp className="h-3 w-3" />
                <span>{data.metrics.likesMes}</span>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs text-emerald-200/60">
              <span>Dia</span>
              <span>Semana</span>
              <span>Mês</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs mt-2">
              <div className="flex items-center gap-1 text-red-300/80">
                ...
                <span>{data.metrics.dislikesDia}</span>
              </div>
              <div className="flex items-center gap-1 text-red-300/80">
                <ThumbsDown className="h-3 w-3" />
                <span>{data.metrics.dislikesSemana}</span>
              </div>
              <div className="flex items-center gap-1 text-red-300/80">
                <ThumbsDown className="h-3 w-3" />
                <span>{data.metrics.dislikesMes}</span>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs text-red-300/60">
              <span>Dia</span>
              <span>Semana</span>
              <span>Mês</span>
            </div>
        ...
        <div className="space-y-4 rounded-2xl border border-sky-400/40 bg-sky-500/10 p-6">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-semibold text-white">
              {data.metrics.sharesDia} / {data.metrics.sharesTotal}
            </p>
            <p className="text-xs text-sky-200/80">
              Total geral: {data.metrics.sharesTotal} compartilhamentos
            </p>
          </div>
